...
async def run_main_agent(
    ...
    input_to_prompt: dict,
    model_name: Optional[str] ,
    message_callback=None,  # Add message callback
) -> Any:
    llm_model = get_llm_model(model_name, output_pydantic_class)
    logger.info(f"Running agent '{agents_name}' with model: {model_name}")
    
    
    # Notify WebSocket clients about the agent starting
    #send_ws_message(message_callback, f"🔍 Running agent in ws2 '{agents_name}'")
    ...
    while attempt < max_attempts and should_retry:
        ...
        try:
            ...
            # Inform WebSockets about the attempt
            #send_ws_message(message_callback, f"Attempt {attempt} for agent in ws3'{agents_name}'")
            ...
            logger.error(f"Processing error: {str(e)}")
            send_ws_message(message_callback, f"Error: {str(e)}")
            should_retry = True
            current_prompt = f"{current_prompt}\n\n[ERROR]: {str(e)}"
    ...
    # if message_callback:
    #     await message_callback(f"🎯 Output for  {agents_name}: {output}")
    if message_callback:
        await message_callback("🎯 Output:")
        await message_callback(f"- agent_msg: {output.agent_msg}")
        if getattr(output, "modified_prompt", ""):  # only send if not empty
           await message_callback(f"- modified_prompt: {output.modified_prompt}")
        if getattr(output, "intent", ""):   
           await message_callback(f"- intent: {output.intent}") 
        if getattr(output, "enhanced_prompt", ""):     
           await message_callback(f"- enhanced_prompt: {output}")
    # Send the final output via WebSocket
