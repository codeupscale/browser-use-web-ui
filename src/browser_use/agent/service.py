...
from src.outputdata.output_data import write_data_to_file
...
def log_response(response: AgentOutput) -> None:
	...
	logger.info(f"🔄 [DEBUG] log_response function ENTERED")
	...
	# Write data to file for browser-use agent
	try:
		logger.info(f"🔄 [DEBUG] log_response function called - starting write_data_to_file")
		logger.info(f"Writing data to file for agent 'Browser Use Agent'")
		...
		for action in response.action:
			if hasattr(action, 'model_dump'):
				serializable_actions.append(action.model_dump())
			elif hasattr(action, 'dict'):
				serializable_actions.append(action.dict())
			else:
				serializable_actions.append(str(action))
		
		# Get final result from the last action if it's a 'done' action
		final_result = None
		for action in response.action:
			if hasattr(action, 'done') and action.done:
				if hasattr(action.done, 'text'):
					final_result = action.done.text
				elif isinstance(action.done, dict) and 'text' in action.done:
					final_result = action.done['text']
				break
		
		logger.info(f"[SERVICE] Writing data to file for agent 'Browser Use Agent'")
		logger.info(f"[SERVICE] Function type: {type(write_data_to_file)}")
		write_data_to_file(
			agents_name="Browser Use Agent",
		...
		logger.info(f"[SERVICE] Successfully called write_data_to_file")
	except Exception as e:
		logger.warning(f"Failed to write browser-use agent data to file: {e}")
